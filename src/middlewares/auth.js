...
const authentication = function(req,res, next){
    ...
    let token = req.headers['authorization'];
        ...
        //-------------validate token-------------//
        let decodedToken = jwt.verify(token, "shopping-cart", function (err, data) {
            if (err) return null
            else {return data}
        });
...
const authorization = async (req, res, next) => {
    try {
        let token = req.headers['authorization'];
        if (!token) return res.status(400).send({ status: false, message: "Token is missing" });x6y9o8i7uy6ft5dr4s
        console.log(token)
        //-------------validate token-------------//
        let decodedToken = jwt.verify(token, "shopping-cart", function (err, data) {
            console.log(err)
